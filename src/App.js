...
class App extends Component {
  ...
  playAgain = () => {
    this.setState({isGameStop: false, score: 0, count: 60})
  ...
  selectedImage = id => {
    ...
    if (imageDetails.id === id) {
      this.setState(prevState => ({
        ...
        isGameStop: prevState.isGameStop,
      }))
      const randomImage = Math.ceil(Math.random() * (imagesList.length - 1))
      const displayImage = imagesList[randomImage]
      this.setState({imageDetails: displayImage})
    } else {
      this.setState(prevState => ({isGameStop: !prevState.isGameStop}))
    }
  }

  renderGame = () => {
    const {imageDetails, activeButton, score, isGameStop} = this.state
    const filterCategory = imagesList.filter(
      eachItem => eachItem.category === activeButton,
    )
    return (
      <div className="bottom-section">
        <div className="game-cart">
          <img src={imageDetails.imageUrl} alt="match" className="image" />
        </div>
        <ul className="buttons-cart">
          {tabsList.map(each => (
            <TabItem
              activateCategory={this.activateCategory}
              key={each.tabId}
              button={each}
              isActive={each.tabId === activeButton}
            />
          ))}
        </ul>
        <ul className="thumbnails-list-container">
          {filterCategory.map(eachThumbnail => (
            <Thumbnails
  ...
  renderGameResult = () => {
    const {count, imageDetails, activeButton, score, isGameStop} = this.state
    this.clearInterVal()
    return (
      <div className="result-cart">
        ...
        <button className="button-style" onClick={this.playAgain}>
          <img
          ...
          <ul className="timer-section">
            ...
            <li className="timer-container">
              <img
                ...
                className="timer"
              />
              <p className="timer-label">{count} sec</p>
            </li>
          </ul>
        </nav>

        <div className="bottom-section-container">
          {!isGameStop ? this.renderGame() : this.renderGameResult()}
        </div>
      </div>
    )
  }
